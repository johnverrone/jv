...
import type { PageProperties } from './notionTypes';
import type { CoffeeBrew, CoffeeRoaster } from './types';
...
export const getRoastersById = async (): Promise<{
	...
	return roasters.results.filter(hasProperties).reduce<{ [id: string]: CoffeeRoaster }>(
		(acc, { id, properties }) => ({
			...acc,
...
const notionResultToCoffeeBrew = (
	...
	const currentlyBrewing = getProperty(properties, CoffeeProperties.currentlyBrewing).checkbox;
	const roasterIds = getProperty(properties, CoffeeProperties.roaster).relation.map((r) => r.id);
	const slug = getProperty(properties, CoffeeProperties.slug).rich_text;
	const rating = getProperty(properties, CoffeeProperties.rating).select?.name ?? '';
	const origin = getProperty(properties, CoffeeProperties.origin)
		.multi_select.map((o) => o.name)
		.join(', ');
	const singleOrigin = getProperty(properties, CoffeeProperties.singleOrigin).formula;
	const flavors = getProperty(properties, CoffeeProperties.flavors).multi_select.map((f) => f.name);
	const process = getProperty(properties, CoffeeProperties.process).select?.name ?? '';
...
const notionResultToRoaster = (id: string, properties: PageProperties): CoffeeRoaster => ({
	id,
	name: getPageTitle(properties)
});
